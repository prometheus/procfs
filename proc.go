...
func (fs FS) AllProcs() (Procs, error) {
	...
	if err != nil {
		return Procs{}, fmt.Errorf("unable to read %q: %w", d.Name(), err)
	}
...
func (p Proc) FileDescriptors() ([]uintptr, error) {
	...
	for i, n := range names {
		...
		if err != nil {
			return nil, fmt.Errorf("unable to read fd %q: %w", n, err)
		}
		fds[i] = uintptr(fd)
	}
...
func (p Proc) fileDescriptors() ([]string, error) {
	...
	names, err := d.Readdirnames(-1)
	if err != nil {
		return nil, fmt.Errorf("unable to read %q: %w", d.Name(), err)
	}
