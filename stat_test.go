...
func TestPareStatFileWithWrongProcessData(t *testing.T) {
	fs, err := NewFS("./testdata/proc1")
	if err != nil {
		t.Fatal(err)
	}
	stat, err := fs.Stat()
	if err == nil {
		t.Errorf("want error but got nil, because the value of process is invalid")
	}
	// cpu
	if want, have := float64(301854)/userHZ, stat.CPUTotal.User; want != have {
		t.Errorf("want cpu/user %v, have %v", want, have)
	}
	if want, have := float64(31)/userHZ, stat.CPU[7].SoftIRQ; want != have {
		t.Errorf("want cpu7/softirq %v, have %v", want, have)
	}

	// intr
	if want, have := uint64(8885917), stat.IRQTotal; want != *have {
		t.Errorf("want irq/total %d, have %d", want, have)
	}
	if want, have := uint64(1), stat.IRQ[8]; want != have {
		t.Errorf("want irq8 %d, have %d", want, have)
	}

	// ctxt
	if want, have := uint64(38014093), stat.ContextSwitches; want != *have {
		t.Errorf("want context switches (ctxt) %d, have %d", want, have)
	}

	// btime
	if want, have := uint64(1418183276), stat.BootTime; want != *have {
		t.Errorf("want boot time (btime) %d, have %d", want, have)
	}

	// processes
	if stat.ProcessCreated != nil {
		t.Errorf("the value of process created should be nil,because it's value is invalid")
	}

	// procs_running
	if want, have := uint64(2), stat.ProcessesRunning; want != *have {
		t.Errorf("want processes running (procs_running) %d, have %d", want, have)
	}

	// procs_blocked
	if want, have := uint64(1), stat.ProcessesBlocked; want != *have {
		t.Errorf("want processes blocked (procs_blocked) %d, have %d", want, have)
	}

	// softirq
	if want, have := uint64(5057579), stat.SoftIRQTotal; want != *have {
		t.Errorf("want softirq total %d, have %d", want, have)
	}

	if want, have := uint64(508444), stat.SoftIRQ.Rcu; want != have {
		t.Errorf("want softirq RCU %d, have %d", want, have)
	}
}
