...
func createNVMeSubsystemFixture(t *testing.T) string {
	...
	// Namespace nvme0n1 (directory entry under subsystem)
	if err := os.MkdirAll(filepath.Join(subsysDir, "nvme0n1"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestNVMeSubsystemMultipleSubsystems(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(subsys0, "nvme0n1"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.MkdirAll(filepath.Join(subsys1, "nvme1n1"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(subsys1, "nvme2n1"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if len(got[0].Namespaces) != 1 || got[0].Namespaces[0] != "nvme0n1" {
		t.Errorf("expected [nvme0n1] for subsys0 namespaces, got %v", got[0].Namespaces)
	}
	...
	if len(got[1].Namespaces) != 2 {
		t.Errorf("expected 2 namespaces for subsys1, got %d", len(got[1].Namespaces))
	}
}
